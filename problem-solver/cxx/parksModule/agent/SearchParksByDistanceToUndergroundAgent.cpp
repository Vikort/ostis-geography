...
#include "SearchParksByDistanceToUndergroundAgent.hpp"
...
namespace parks {
  ...
  SC_AGENT_IMPLEMENTATION(SearchParksByDistanceToUndergroundAgent)
  {
    if (!edgeAddr.IsValid()) {
    ...
    ScAddr answer = parksByPropertyInNumericalRangeFinder->
        ...
        ms_context.get(),
        questionNode,
        ParksKeynodes::concept_distance,
        ParksKeynodes::nrel_distance_to_nearest_metro_station);
