...
#include "SearchParksByFoundingYearAgent.hpp"
...
namespace parks {
  ...
  SC_AGENT_IMPLEMENTATION(SearchParksByFoundingYearAgent)
  {
    if (!edgeAddr.IsValid()) {
    ...
    ScAddr answer = parksByPropertyInNumericalRangeFinder->
        ...
        ms_context.get(),
        questionNode,
        ParksKeynodes::concept_year,
        ParksKeynodes::nrel_founding_year);
