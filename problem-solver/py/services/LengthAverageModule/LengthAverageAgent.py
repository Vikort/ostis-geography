...
class LengthAverageAgent(ScAgent):
    ...
    def RunImpl(self, evt: ScEventParams) -> ScResult:
        ...
        if self.module.ctx.HelperCheckEdge(
                self.keynodes['action_get_average_length'],
                self.main_node,
            ...
            try:
                ...
                while(definitionIterator.Next()):
                    ...
                    translationIterator = self.ctx.Iterator5(
                        ...
                        ScType.NodeConst,
                        ScType.EdgeAccessConstPosPerm,
                        self.keynodes['nrel_street_length']
                    )
                    ...
                    if translationIterator.Next():
                        ...
                        if valueterator.Next():
                            all_length+=int(self.ctx.GetLinkContent(valueterator.Get(2)).AsString())
                            street_count+=1

                linkAddr = self.ctx.CreateLink()
                self.ctx.SetLinkContent(linkAddr, str(int(all_length/street_count)))
