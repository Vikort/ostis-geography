...
class DistanceAgent(ScAgent):
    ...
    def RunImpl(self, evt: ScEventParams) -> ScResult:
        ...
        if self.module.ctx.HelperCheckEdge(
            ...
            try:
                ...
                bus1 = self.get_action_argument(self.main_node, 'rrel_1')
                bus2 = self.get_action_argument(self.main_node, 'rrel_2')
                ...
                if defIter.Next():
                    ...
                    if defIter.Next():
                        city2 = defIter.Get(2)
                        distance = self.getDistance([self.get_main_idtf(bus1), self.get_main_idtf(city1)],
                                                    [self.get_main_idtf(bus2), self.get_main_idtf(city2)])
                        ...
                        if self.ctx.SetLinkContent(answer, distance):
                            ...
                            self.add_nodes_to_answer(answerNode, [answer, bus1, bus2, self.keynodes['nrel_distance'], tuple, query_edge, query_edge_1, query_edge_11, query_edge2])
                else:
                    raise Exception("Invalid content: " + bus1 + " or " + bus2 + "cannot find")
    ...
    def getDistance(self, bus1, bus2):
        url = "https://www.openstreetmap.org/geocoder/search_osm_nominatim?query=" + bus1[0] + "%2C" + bus1[1]
        coord1 = self.getLatLon(url)
        url_n = "https://www.openstreetmap.org/geocoder/search_osm_nominatim?query=" + bus2[0] + "%2C" + bus2[1]
