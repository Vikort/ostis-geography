...
class CountAverageAgent(ScAgent):
    ...
    def RunImpl(self, evt: ScEventParams) -> ScResult:
        ...
        if self.module.ctx.HelperCheckEdge(
                self.keynodes['action_get_average_count'],
                self.main_node,
            ...
            try:
                ...
                definitionIterator = self.ctx.Iterator3(
                    ScType.Unknown,
                ...
                while(definitionIterator.Next()):
                    street = definitionIterator.Get(0) # это улица
                    ...
                    translationIterator = self.ctx.Iterator5(
                        ...
                        ScType.NodeConst,
                        ScType.EdgeAccessConstPosPerm,
                        self.keynodes['nrel_number_of_house']
                    )


                    if translationIterator.Next():
                        numberName = translationIterator.Get(2)
                        valueterator = self.ctx.Iterator3(
                            numberName,
                            ScType.EdgeDCommonConst,
                            ScType.Link
                        )
                        if valueterator.Next():
                            all_number_houses+=int(self.ctx.GetLinkContent(valueterator.Get(2)).AsString())
                            street_count+=1

                linkAddr = self.ctx.CreateLink()
                self.ctx.SetLinkContent(linkAddr, str(int(all_number_houses/street_count)))
                ...
                self.add_nodes_to_answer(answerNode, [linkAddr, edge])
