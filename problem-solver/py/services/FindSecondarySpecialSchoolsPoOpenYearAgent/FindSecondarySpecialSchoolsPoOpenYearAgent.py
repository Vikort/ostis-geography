...
class FindSecondarySpecialSchoolsPoOpenYearAgent(ScAgent):
    ...
    def RunImpl(self, evt: ScEventParams) -> ScResult:
        ...
        if self.module.ctx.HelperCheckEdge(
                self.keynodes['action_find_secondary_special_schools_po_open_year'],
                self.main_node,
            ...
            try:
                ...
                secondary_school_iterator = self.ctx.Iterator5(
                    ScType.Unknown,
                    ScType.EdgeDCommon,
                    node,
                    ScType.EdgeAccessConstPosPerm,
                    self.keynodes['nrel_open_year']
                )
                while secondary_school_iterator.Next():
                    secondary_special_school = secondary_school_iterator.Get(0)
                    checkIterator = self.ctx.Iterator3(
                        self.keynodes['concept_secondary_schools'],
						ScType.EdgeAccessConstPosPerm,
                        secondary_special_school
					)
                    if checkIterator.Next():
                        self.add_nodes_to_answer(answerNode, [secondary_special_school, secondary_school_iterator.Get(1), secondary_school_iterator.Get(2) , secondary_school_iterator.Get(3), secondary_school_iterator.Get(4)])
