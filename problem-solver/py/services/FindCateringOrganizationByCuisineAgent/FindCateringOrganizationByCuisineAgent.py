...
class FindCateringOrganizationByCuisineAgent(ScAgent):
    ...
    def RunImpl(self, evt: ScEventParams) -> ScResult:
        ...
        if self.module.ctx.HelperCheckEdge(
                self.keynodes['action_find_catering_organization_by_cuisine'],
                self.main_node,
            ...
            try:
                ...
                cateting_organization_iterator = self.ctx.Iterator5(
                    ...
                    node,
                    ScType.EdgeAccessConstPosPerm,
                    self.keynodes['nrel_cuisine']
                )
                while cateting_organization_iterator.Next():
                    catering_special_organization = cateting_organization_iterator.Get(0)
                    checkIterator = self.ctx.Iterator3(
                        self.keynodes['concept_cafe'],
						ScType.EdgeAccessConstPosPerm,
                        catering_special_organization
					)
