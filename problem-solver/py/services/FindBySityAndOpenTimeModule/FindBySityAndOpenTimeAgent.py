...
class FindBySityAndOpenTimeAgent(ScAgent):
    ...
    def RunImpl(self, evt: ScEventParams) -> ScResult:
        ...
        if self.module.ctx.HelperCheckEdge(
                self.keynodes['action_find_by_sity_and_open_time'],
                self.main_node,
            ...
            try:
                ...
                node1 = self.get_action_argument(self.main_node, 'rrel_2')
                answerNode = self.ctx.CreateNode(ScType.NodeConstStruct)  # создаем узел ответа
                self.add_nodes_to_answer(answerNode, [node, node1])  # добавляем входные аргументы в ответ
                ...
                while (definitionIterator.Next()):
                    ...
                    librIterator = self.ctx.Iterator3(  # находит принадлежность к библиотеке
                        self.keynodes['concept_abstract_library'],
                        ScType.EdgeAccessConstPosPerm,
                        townElement,
                    )
                    if librIterator.Next():
                        library = librIterator.Get(2)
                        timeIterator = self.ctx.Iterator5(  # находит всё что есть в городе
                            library,
                            ScType.EdgeDCommon,
                            node1,
                            ScType.EdgeAccessConstPosPerm,
                        ...
                        if timeIterator.Next():
                            self.add_nodes_to_answer(answerNode, [townElement, definitionIterator.Get(1), definitionIterator.Get(2)])
