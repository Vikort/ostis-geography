...
class GetDrivingSchoolsByFailureRateAgent(ScAgent):
    ...
    def RunImpl(self, evt: ScEventParams) -> ScResult:
        ...
        if self.module.ctx.HelperCheckEdge(
                self.keynodes['action_get_driving_schools_by_failure_rate'],
                self.main_node,
            ...
            try:
                ...
                drSchoolIterator = self.ctx.Iterator5(
                    ...
                    node,
                    ScType.EdgeAccessConstPosPerm,
                    self.keynodes['nrel_failure_rate']
                )
                while drSchoolIterator.Next():
                    drSchool = drSchoolIterator.Get(0)
                    # проверка на принадлежность к типу автошкол
                    drSchoolcheck = False
