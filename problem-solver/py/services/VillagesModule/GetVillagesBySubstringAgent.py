...
class GetVillagesBySubstringAgent(ScAgent):
    ...
    def RunImpl(self, evt: ScEventParams) -> ScResult:
        ...
        if self.module.ctx.HelperCheckEdge(
                self.keynodes['action_get_villages_by_substring'],
                self.main_node,
            ...
            try:
                ...
                if villageIterator.Next():
                    ...
                    while nameIterator.Next():
                        substring = self.ctx.GetLinkContent(node).AsString()
                        village_name = self.ctx.GetLinkContent(nameIterator.Get(2)).AsString()

                        if substring in village_name:
                            for i in range(5):
