...
class GetVillagesByFirstCharacterAgent(ScAgent):
    ...
    def RunImpl(self, evt: ScEventParams) -> ScResult:
        ...
        if self.module.ctx.HelperCheckEdge(
                self.keynodes['action_get_villages_by_first_character'],
                self.main_node,
            ...
            try:
                ...
                if villageIterator.Next():
                    ...
                    while nameIterator.Next():
                        first_char = self.ctx.GetLinkContent(node).AsString()
                        village_name = self.ctx.GetLinkContent(nameIterator.Get(2)).AsString()

                        if len(village_name) > 0 and first_char == village_name[0]:
                            for i in range(5):
