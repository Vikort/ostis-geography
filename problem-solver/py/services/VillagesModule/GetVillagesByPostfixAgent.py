...
class GetVillagesByPostfixAgent(ScAgent):
    ...
    def RunImpl(self, evt: ScEventParams) -> ScResult:
        ...
        if self.module.ctx.HelperCheckEdge(
                self.keynodes['action_get_villages_by_postfix'],
                self.main_node,
            ...
            try:
                ...
                if villageIterator.Next():
                    ...
                    while nameIterator.Next():
                        postfix = self.ctx.GetLinkContent(node).AsString()
                        village_name = self.ctx.GetLinkContent(nameIterator.Get(2)).AsString()

                        if village_name.endswith(postfix):
                            for i in range(5):
