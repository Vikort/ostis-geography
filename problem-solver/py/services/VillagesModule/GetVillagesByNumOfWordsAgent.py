...
class GetVillagesByNumOfWordsAgent(ScAgent):
    ...
    def RunImpl(self, evt: ScEventParams) -> ScResult:
        ...
        if self.module.ctx.HelperCheckEdge(
                self.keynodes['action_get_villages_by_num_of_words'],
                self.main_node,
            ...
            try:
                ...
                villageIterator = self.ctx.Iterator3(
                    self.keynodes['concept_village'],
                    ScType.EdgeAccessConstPosPerm,
                    ScType.Unknown
                )

                if villageIterator.Next():
                    village = villageIterator.Get(0)
                    # пятиэлементный итератор для поиска конструкции ... <= nrel_sc_text_translation: ...;;
                    nameIterator = self.ctx.Iterator5(
                        village,
                        ScType.EdgeDCommon,
                        ScType.Unknown,
                        ScType.EdgeAccessConstPosPerm,
                    ...
                    while nameIterator.Next():
                        postfix: str = self.ctx.GetLinkContent(node).AsString()
                        village_name: str = self.ctx.GetLinkContent(nameIterator.Get(2)).AsString()

                        if postfix.isnumeric() and len(village_name.split()) == int(postfix):
                            for i in range(5):
                                self.add_nodes_to_answer(
                                    answerNode,
                                    [nameIterator.Get(i)]
                                )
