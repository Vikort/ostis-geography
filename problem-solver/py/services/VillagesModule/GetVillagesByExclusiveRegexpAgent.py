...
class GetVillagesByExclusiveRegexpAgent(ScAgent):
    ...
    def RunImpl(self, evt: ScEventParams) -> ScResult:
        ...
        if self.module.ctx.HelperCheckEdge(
                self.keynodes['action_get_villages_by_exclusive_regexp'],
                self.main_node,
            ...
            try:
                ...
                if villageIterator.Next():
                    ...
                    while nameIterator.Next():
                        pattern = self.ctx.GetLinkContent(node).AsString()
                        village_name = self.ctx.GetLinkContent(nameIterator.Get(2)).AsString()

                        if not re.search(pattern=pattern, string=village_name):
                            for i in range(5):
