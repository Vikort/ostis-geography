...
var setSearchVillagesByDistrictViewerWindow = function (sandbox) {
    ...
    $('#series-tools-' + sandbox.container).load('static/components/html/search_villages_by_district_component.html', function () {
        SCWeb.core.Server.resolveScAddr(keynodes, function (keynodes) {
            SCWeb.core.Server.resolveIdentifiers(keynodes, function (idf) {
                var buttonSearch = idf[keynodes['ui_search_villages_by_district_in_memory']];

                $(buttonFind).html(buttonSearch);
                $(buttonFind).click(function () {
                    var districtString = $(inputSeasons).val();

                    console.log("Villages find district string" + districtString);
    ...
    this.applyTranslation = function (namesMap) {
        SCWeb.core.Server.resolveScAddr(keynodes, function (keynodes) {
            SCWeb.core.Server.resolveIdentifiers(keynodes, function (idf) {
                var buttonLoad = idf[keynodes['ui_search_villages_by_district_in_memory']];
...
function findVillages(searchParams) {
    ...
    SCWeb.core.Server.resolveScAddr([searchParams.District], function (keynodes) {
        ...
        SCWeb.core.Server.resolveScAddr(["ui_menu_get_villages_by_district"], function (data) {
            var cmd = data["ui_menu_get_villages_by_district"];
            console.log("cmd", cmd);
