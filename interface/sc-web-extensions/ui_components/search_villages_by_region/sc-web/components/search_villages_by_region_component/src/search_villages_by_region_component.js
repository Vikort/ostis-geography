SearchVillagesByRegionComponent = {
    ext_lang: 'search_villages_by_region_code',
    formats: ['format_search_villages_by_region_json'],
    struct_support: true,

    factory: function (sandbox) {
        return new setSearchVillagesByRegionViewerWindow(sandbox);
    }
};

var setSearchVillagesByRegionViewerWindow = function (sandbox) {
    ...
    var keynodes = ['ui_search_villages_by_region_in_memory'];

    $('#' + sandbox.container).prepend('<div class="inputBox" id="series-tools-' + sandbox.container + '"></div>');
    $('#series-tools-' + sandbox.container).load('static/components/html/search_villages_by_region_component.html', function () {
        SCWeb.core.Server.resolveScAddr(keynodes, function (keynodes) {
            SCWeb.core.Server.resolveIdentifiers(keynodes, function (idf) {
                var buttonSearch = idf[keynodes['ui_search_villages_by_region_in_memory']];

                $(buttonFind).html(buttonSearch);
                $(buttonFind).click(function () {
                    var RegionString = $(inputSeasons).val();

                    console.log("Villages find Region string" + RegionString);
    ...
    this.applyTranslation = function (namesMap) {
        SCWeb.core.Server.resolveScAddr(keynodes, function (keynodes) {
            SCWeb.core.Server.resolveIdentifiers(keynodes, function (idf) {
                var buttonLoad = idf[keynodes['ui_search_villages_by_region_in_memory']];
...
function findVillages(searchParams) {
    ...
    SCWeb.core.Server.resolveScAddr([searchParams.Region], function (keynodes) {
        ...
        SCWeb.core.Server.resolveScAddr(["ui_menu_get_villages_by_region"], function (data) {
            var cmd = data["ui_menu_get_villages_by_region"];
            console.log("cmd", cmd);
